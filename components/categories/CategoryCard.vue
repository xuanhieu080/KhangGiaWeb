<template>
    <UCard
        ...
        <NuxtLink :to="category.link">
            <img loading="lazy" format="webp" :src="category.image" class="w-full h-full" :alt="category.name" />
        </NuxtLink>
    </UCard>
...
.category-card {
    ...
    &:after {
        ...
        left: 0;
        pointer-events: none;
        @apply bg-transparent transition ease-in-out duration-300;
    }
    &:hover {
        &:after {
            @apply bg-black/20 transition ease-in-out duration-300;
        }
    }
}
</style>
